...
public class SubContractorBillingActivity extends BaseAppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent.hasExtra(AppKeys.PROJECT)) {
            selected_project = intent.getParcelableExtra(AppKeys.PROJECT);
        }
    ...
    private void setupAdapter() {
        mAdapter = new SubContractorSpinnerAdapter(mList, SubContractorBillingActivity.this);
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                try {
                    ...
                    if (bill_amount >= paid_amount) {
                        ...
                        Toast.makeText(SubContractorBillingActivity.this, "Please add proper amount", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception exception) {
                    Log.e("Error", "" + exception.getMessage());
                }
        ...
        RetrofitClient.getApiService().storeClientBillBySubContractorProjectId(true, bill.id, bill.percentage, bill.amount, bill.remark, bill.balance, bill.paid, bill.payment_date, bill.billing_date, selected_project.id, selected_sub_contractor.id,selected_project.contractor_id).enqueue(new Callback<BillResponseModel>() {
            ...
            public void onResponse(Call<BillResponseModel> call, Response<BillResponseModel> response) {
                if (response.code() == 200) {
                    ...
                    setGrandTotal();
                    clearEdit();
                    Toast.makeText(SubContractorBillingActivity.this, "Successfully edited bill", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(SubContractorBillingActivity.this, "Error to edit bill", Toast.LENGTH_SHORT).show();
                }
                stopLoader();
            }
            ...
            public void onFailure(Call<BillResponseModel> call, Throwable t) {
                Toast.makeText(SubContractorBillingActivity.this, "Error " + t.getMessage(), Toast.LENGTH_SHORT).show();
                stopLoader();
            }
        });
...
//        Log.e(pdAmount +"/"+ pAmount,pPercentage+"="+pdPercentage);
    ...
    public void onBackPressed() {
        ...
        bundle.putParcelable(AppKeys.PROJECT, selected_project);
